<?php
	...
	if (isset($_GET['uid'])) {
		if (!isset($_GET['convo_id'])) {
			$cdata = " user_ids = '{$_GET['uid']},{$_SESSION['id']}' ";
			$cdata2 = " user_ids = '{$_SESSION['id']},{$_GET['uid']}' ";
		...
		$user = $link->query("SELECT * FROM customer where id in ({$thread['user_ids']}) and id !=" . $_SESSION['id'])->fetch_array();
	}
?>
