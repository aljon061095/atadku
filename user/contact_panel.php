...
<div class="container-fluid pt-3 shadow" id="contact-panel">
	...
	<ul class="list-group" id="convo-list">
			<?php
				$thread = $link->query("SELECT * from thread where concat('[',REPLACE(user_ids,',','],['),']') like '%[{$_SESSION['id']}]%' ");
				while ($row = $thread->fetch_array()) :
					$user = $link->query("SELECT * FROM customer where id in ({$row['user_ids']}) and id!= {$_SESSION['id']} ")->fetch_array();
			...
			<li class="list-group-item w-100" data-id="<?php echo md5($row['id']) ?>">
				<table class="" width="100%">
					<tr>
						<td width="10%">
							<div class="uavatar mr-2"><img src="../uploads/<?php echo $user['profile']; ?>" alt=""></div>
						</td>
						<td>
							<strong><?php echo ucwords($user['id']) ?></strong>
		...
		$.ajax({
			...
			complete: () => {
				$('.searched-user').click(function() {
					console.log('test')
					location.href = "home.php?page=conversation&uid=" + $(this).attr('data-id')
				})
			}
		})
	})
	$('#convo-list li').click(function() {
		location.href = "home.php?page=conversation&convo_id=" + $(this).attr('data-id')
	})
